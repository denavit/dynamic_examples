...
def boltText_output(bolt, UTSs_str,UTSn_str):
    ...
    text.newline(r'$D_{1,bsc} = d_{1,bsc} = d_{bsc} - 1.25H = ' + sigfigstr(boltObj.dbsc) + r'\text{ in. }- 1.25\left(' + sigfigstr(boltObj.H) + r'\text{ in.}\right) = ' + sigfigstr(boltObj.D1bsc) + r'$ in.', tag='p',cls='eqn')
    text.newline(r'$D_{2,bsc} = d_{2,bsc} = d_{bsc} - 0.75H = ' + sigfigstr(boltObj.dbsc) + r'\text{ in. }- 0.75\left(' + sigfigstr(boltObj.H) + r'\text{ in.}\right) = ' + sigfigstr(boltObj.D2bsc) + r'$ in.', tag='p',cls='eqn')
    ...
    text.newline(r'$AS_{s,max} = \pi\dfrac{3}{4}D_{1,bsc}LE = \pi\dfrac{3}{4}\left(' + sigfigstr(boltObj.D1bsc) + r'\text{ in.}\right)LE = ' + sigfigstr(boltObj.ASs_max_FEDSTD_6b()) + r'(LE)\text{ in.}^{2}$', tag='p',cls='eqn')
    ...
    line += r'&= ' + sigfigstr(boltObj.ASn_min_FEDSTD_2a()) + r'(LE)\text{ in.}^{2}\end{aligned}$'
    text.newline(line, tag='p',cls='eqn')

    text.newline(f'Calculate ' + r'$R_{1}$', tag='p')
    text.newline(r'$R_{1} = \dfrac{AS_{s,max}}{AS_{n,min}} = \dfrac{' + sigfigstr(boltObj.ASs_max_FEDSTD_6b()) + r'(LE)\text{ in.}^{2}}{' + sigfigstr(boltObj.ASn_min_FEDSTD_2a()) + r'(LE)\text{ in.}^{2}} = ' + sigfigstr(boltObj.ASs_max_FEDSTD_6b()/boltObj.ASn_min_FEDSTD_2a()) + r'$', tag='p',cls='eqn')
    ...
    if (boltObj.ASs_max_FEDSTD_6b() / boltObj.ASn_min_FEDSTD_2a()) / (boltObj.UTSn / boltObj.UTSs) <= 1:
        text.newline(f'Since ' + r'$\frac{R_{1}}{R_{2}}$' + f' is less than or equal to 1, the external threads are weaker than the interal threads and the required length of engagement should be based on the \
                    ...
                    equal (i.e., ' + r'$\frac{R_{1}}{R_{2}}$' +  f' is near 1) then a combined shear failure may control. Formula (13) gives the required length of engagement based upon combined shear failure of external and internal \
        ...
        text.newline(r'$\frac{AS_{s,min}}{LE} = \pi D_{1,max} n\left(\dfrac{1}{2n} + \dfrac{1}{\sqrt{3}}\left(d_{2,min} - D_{1,max}\right)\right) = \pi\left(' + str(boltObj.D1max) + r'\text{ in.}\right)\left(' + str(boltObj.n) + r'\text{ threads/in.}\right)\left(\dfrac{1}{2\left(' + str(boltObj.n) + r'\text{ threads/in.}\right)} + \dfrac{1}{\sqrt{3}}\left(' + str(boltObj.d2min) + r'\text{ in.} - ' + str(boltObj.D1max) + r'\text{ in.}\right)\right) = ' + sigfigstr(boltObj.ASs_min_FEDSTD_4a()) + r'\text{ in.}^{2}$', tag='p',cls='eqn')
        text.newline(f'The required length of engagement is computed using formula (14) as follows:', tag='p')
        text.newline(r'$LE = \dfrac{2A_{s}}{AS_{s,min}} = \dfrac{2\left(' + sigfigstr(boltObj.As_FEDSTD_1a()) + r'\text{ in.}^{2}\right)}{' + sigfigstr(boltObj.ASs_min_FEDSTD_4a()) + r'\text{ in.}^{2}} = ' + sigfigstr(boltObj.LEr_FEDSTD_14()) + r'\text{ in.}$', tag='p',cls='eqn')
        ...
        text.newline(f'Since ' + r'$\frac{R_{1}}{R_{2}}$' + f' is greater than 1, the internal threads are weaker than the external threads and the required length of engagement should be based on the strength of the \
                     ...
                     (i.e., ' + r'$\frac{R_{1}}{R_{2}}$' +  f'then a combined shear failure may control. Formula (13) gives the required length of engagement based upon combined shear failure of external and internal \
                     threads. FED-STD-H28/2B does not provide an explicit limit for when formula (13) applies. The required length of engagement from both formulas are calculated in this example.', tag='p')
        text.newline(f'The required length of engagement based upon shear of internal threads is defined in formula (16) as the length of engagement based upon developing full tensile strength of external threads \
                     with threads a basic size, calculated using formula (15), multiplied by ' + r'$\frac{R_{1}}{R_{2}}$' + f'.', tag='p')
        ...
        text.newline(r'$LE_{r,16} = \dfrac{\dfrac{2A_{s}}{AS_{n,min}}}{R_{2}} = \dfrac{\dfrac{2\left(' + sigfigstr(boltObj.As_FEDSTD_1a()) + r'\text{ in.}^{2}\right)}{' + sigfigstr(boltObj.ASn_min_FEDSTD_2a()) + r'\text{ in.}^{2}}}{' + sigfigstr(boltObj.UTSn / boltObj.UTSs) + r'} = ' + sigfigstr(boltObj.LEr_FEDSTD_16()) + r'\text{ in.}$', tag='p',cls='eqn')
        text.newline(f'The required length of engagement based upon combined shear failure of external and internal threads, which is defined as four times the tensile stress area divided by ' + r'$\pi$' + f' times the basic pitch diameter, is computed using formula (13) as follows: ', tag='p')
        text.newline(r'$LE_{r,13} = \dfrac{4A_{s}}{\pi d_{2,bsc}} = \dfrac{4\left(' + sigfigstr(boltObj.As_FEDSTD_1a()) + r'\text{ in.}^{2}\right)}{\pi\left(' + sigfigstr(boltObj.d2bsc) + r'\text{ in.}\right)} = ' + sigfigstr(boltObj.LEr_FEDSTD_13()) + r'\text{ in.}$', tag='p',cls='eqn')
