...
def boltText_output(bolt, UTSs_str,UTSn_str,LE_str):
    ...
    text.newline(r'$= \pi\left(\dfrac{' + sigfigstr(boltObj.d2bsc) + r'\text{ in.}}{2} - \dfrac{3(' + sigfigstr(boltObj.H) + r'\text{ in.})}{16}\right)^{2}$', tag='p') 
    ...
    text.newline(r'$F = UTSs\left(A_{s,1a}\right) = ' + str(boltObj.UTSs) + r'\frac{\text{lbs}}{\text{in.}^{2}}\left(' + sigfigstr(boltObj.As_FEDSTD_1a()) + r'\text{ in.}^{2}\right) = ' + sigfigstr(boltObj.UTSs * boltObj.As_FEDSTD_1a()) + r'\text{ lbs}$', tag='p')
    ...
    text.newline(r'$= \dfrac{\pi\dfrac{3}{4}\left(' + sigfigstr(boltObj.D1bsc) + r'\text{ in.}\right)}{\pi\left(' + str(boltObj.dmin) + r'\text{ in.}\right)\left(\dfrac{1}{2\left(' + str(boltObj.n) + r'\text{ threads/in.}\right)} + \dfrac{1}{\sqrt{3}}\left(' + str(boltObj.dmin) + r'\text{ in.} - ' + str(boltObj.D2max) + r'\text{ in.}\right)\right)}$', tag='p')   
    # Intermediate Step
    text.newline(r'$ = ' + sigfigstr(boltObj.ASs_max_FEDSTD_6b() / boltObj.ASn_min_FEDSTD_2a()) + r'$', tag='p')
    text.newline(r'$R_{2} = \dfrac{UTSn}{UTSs} = ' + sigfigstr(boltObj.UTSn / boltObj.UTSs) + '$', tag='p') # Add intermediate step
    text.newline(r'$\dfrac{R_{1}}{R_{2}} = ' + sigfigstr((boltObj.ASn_min_FEDSTD_2a() / boltObj.ASs_max_FEDSTD_6b()) / (boltObj.UTSn / boltObj.UTSs)) + r'$', tag='p')  # Add intermediate step
    ...
    if (boltObj.ASn_min_FEDSTD_2a() / boltObj.ASs_max_FEDSTD_6b()) / (boltObj.UTSn / boltObj.UTSs) < 1:
        ...
        text.newline(r'$AS_{s,min} = \pi d_{min}\left(\dfrac{1}{2n} + \dfrac{1}{\sqrt{3}}\left(d_{min} - D_{2,max}\right)\right)LE = \pi\left(' + str(boltObj.dmin) + r'\text{ in.}\right)\left(\dfrac{1}{2\left(' + 
                     str(boltObj.n) + r'\text{ threads/in.}\right)} + \dfrac{1}{\sqrt{3}}\left(' + str(boltObj.dmin) + r'\text{ in.} - ' + str(boltObj.D2max) + r'\text{ in.}\right)\right)\left(' + str(LE) + r'\text{ in.}\right) = ' + sigfigstr(boltObj.ASn_min_FEDSTD_2a()) + r'\text{ in.}^{2}$', tag='p')
        text.newline(r'$LE = \dfrac{2A_{s}}{AS_{s,min}} = \dfrac{2\left(' + sigfigstr(boltObj.As_FEDSTD_1a()) + r'\text{ in.}^{2}\right)}{' + sigfigstr(boltObj.ASn_min_FEDSTD_2a()) + r'\text{ in.}^{2}} = ' + sigfigstr(2 * boltObj.As_FEDSTD_1a() / boltObj.ASn_min_FEDSTD_2a()) + r'\text{ in.}$', tag='p')
    else:
        ...
        text.newline(r'$LE_{r,16} = \dfrac{\dfrac{2A_{s}}{AS_{n}}}{R_{2}} = \dfrac{\dfrac{2\left(' + sigfigstr(boltObj.As_FEDSTD_1a()) + r'\text{ in.}^{2}\right)}{' + sigfigstr(boltObj.ASn_min_FEDSTD_2a()) + r'\text{ in.}^{2}}}{' + sigfigstr(boltObj.UTSn / boltObj.UTSs) + r'} = ' + sigfigstr(boltObj.LEr_FEDSTD_16()) + r'\text{ in.}$', tag='p')
