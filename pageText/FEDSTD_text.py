...
def boltFEDText(bolt, UTSs_str, UTSn_str):
    ...
    def boltText_input(UTSs_str,UTSn_str):
        ...
        text.newline(r'<label class="input_label" for="boltDropdown">Nominal Size and Threads per Inch:</label>')
        text.newline(r'<select class = "input_dropdown" id="boltDropdown" style="clear: right;" name="boltDropdown">')
        ...
        text.newline(r'<label class="input_units">psi</label>')

        text.newline(r'<label class="input_label" for="UTSn">$UTS_n$ (Strength of Internally Threaded Part):</label>')
        text.newline(r'<input class="input_textbox" type="text" id="UTSn" value=' + f'"{UTSn_str}"' + r' name="UTSn"/>')
        text.newline(r'<label class="input_units">psi</label>')
    ...
    def boltText_output(bolt, UTSs_str,UTSn_str):
        ...
        text.newline(f'According to formula (8), ' + r'$R_{1}$' + f' is the ratio of ' + r'$AS_{s,max}$' + f' to ' + r'$AS_{n,min}$' + f'. Both ' + r'$AS_{s,max}$' + f' and ' + r'$AS_{n,min}$' + f' depend on LE, which cancels out.', tag='p')
        ...
        text.newline(f'According to formula (9), '+ r'$R_{2}$' + f' is the ratio of ' + r'$UTS_{n}$' + f' to ' + r'$UTS_{s}$' + f'.',tag='p')
        ...
        if (boltObj.ASs_max_FEDSTD_6b() / boltObj.ASn_min_FEDSTD_2a()) / (boltObj.UTSn / boltObj.UTSs) <= 1:
            ...
            text.newline(f'The tensile stress area, ' + r'$A_{s}$' + f', can be computed from either formula (1a) or (1b) and was computed previously.', tag='p')
            text.newline(r'$A_{s}' + f' = ' + sigfigstr(boltObj.As_FEDSTD_1a()) + r'\text{ in.}^{2}$',tag='p',cls='eqn')
            text.newline(f'The shear area, ' + r'$AS_{s}$' + f', can be computed from either formula (4a) or (4b) and was computed previously.',tag='p')
